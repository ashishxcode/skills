...
> This project uses **React + Vite + TanStack Query**, not Next.js + SWR.
> Skip the following rules that do not apply:
>
> - **Section 3 (Server-Side Performance)**: All 5 `server-*` rules (`React.cache()`, RSC boundaries, `after()`, etc.)
> - **Rule 4.2 (`client-swr-dedup`)**: Recommends SWR — use TanStack Query instead
> - **Rule 2.4 (`bundle-dynamic-imports`)**: References `next/dynamic` — use `React.lazy()` instead

---

## Abstract

Comprehensive performance optimization guide for React and Next.js applications, designed for AI agents and LLMs. Contains 37 rules across 8 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.
...
import { TbSearch, TbX, TbMenu } from "react-icons/tb";
// react-icons barrel loads entire icon family
```
...
// react-icons: already tree-shaken per-family when imported directly
import { TbSearch } from "react-icons/tb";
```

**Alternative: Vite — optimizeDeps**
...
import { Check, X, Menu } from "lucide-react";
// Pre-bundled by Vite at dev startup
```

Direct imports provide 15-70% faster dev boot, 28% faster builds, 40% faster cold starts, and significantly faster HMR.

Libraries commonly affected: `lucide-react`, `@tabler/icons-react`, `react-icons`, `@headlessui/react`, `@radix-ui/react-*`, `lodash`, `ramda`, `date-fns`, `rxjs`, `react-use`.
